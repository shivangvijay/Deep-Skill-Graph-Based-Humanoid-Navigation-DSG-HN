...
#define SCENE_FILE          "../config/scene/ai_maker_space_scene.xml"
...
int main(int argc, char **argv)
{
    ...
    // Torch model (opaque)
    auto ctx = mpc_create(ckpt_path, norm_path, history, d_model, n_heads, n_layers);
    ...
    auto [pos, quat] = robot_bridge->generateRandomPos();
    ...
    while (running)
    {
        ...
        if (kb.key() == "r" || (has_joystick && js.button_[6])) {
            auto [p, q] = robot_bridge->generateRandomPos();
